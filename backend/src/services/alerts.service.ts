...
export async function getCityZones(): Promise<FeatureCollection<Geometry, GeoJsonProperties>> {
    if (!citiesSingleton) {
        const raw = await readFile(CITIES_GEOJSON_PATH, "utf-8");
        citiesSingleton = JSON.parse(raw) as FeatureCollection<Geometry, GeoJsonProperties>;
    }
